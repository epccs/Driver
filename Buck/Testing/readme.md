...
These tests are for an assembled Buck board 20226^0 which may be referred to as a Unit Under Test (UUT). If the UUT fails and can be reworked then do so, otherwise it needs to be scraped. 
...
Setup a current limited supply with 12V and about 30mA limit. Connect the supply to the input. Measure the input current and output voltage.
